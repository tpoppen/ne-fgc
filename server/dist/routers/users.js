...
UsersRouter
    // admin only
    .get('/', [authMiddleware, addPermissionsMiddleware], async (req, res) => {
    ...
    // admin only
    .put('/:userId/permissions', [authMiddleware, addPermissionsMiddleware], async (req, res) => {
    if (!req.permissions.includes(PERMISSIONS.USER_ADMIN)) {
        return res.status(403).send({ message: "Not Authorized" });
    }
    try {
    ...
    .get('/:userId', [authMiddleware, addPermissionsMiddleware], async (req, res) => {
    ...
    // admin or matching user
    .put('/:userId', [authMiddleware, addPermissionsMiddleware], async (req, res) => {
    if (req.permissions.includes(PERMISSIONS.USER_ADMIN)) {
        try {
    ...
    try {
        ...
        if (error.$response) {
            return res.status(error.$response.statusCode).send({ message: error.message });
        }
        res.status(500).send({ message: "An unexpected error occurred" });
    }
})
    // admin or matching user
    .delete('/:userId', [authMiddleware, addPermissionsMiddleware], async (req, res) => {
    if (req.permissions.includes(PERMISSIONS.USER_ADMIN)) {
        try {
